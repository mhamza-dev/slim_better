...
interface WhatsAppButtonProps {
    ...
    messageType?: 'greeting' | 'appointment'
    appointmentDetails?: {
        date: string
        time?: string
...
export function WhatsAppButton({
    ...
    const handleClick = () => {
        ...
        if (messageType === 'appointment' && appointmentDetails) {
    ...
    const baseClasses = `inline-flex items-center justify-center rounded-full transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 ${sizeClasses[size]} ${className}`
    ...
    return (
        <button
            ...
            title={`Message ${patientName || 'patient'} on WhatsApp`}
            aria-label={`Open WhatsApp chat with ${patientName || 'patient'}`}
        >
